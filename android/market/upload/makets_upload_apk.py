...
from android.market.upload.enums import MarketChannel, AppType
...
# app类型
app_type = AppType.penguin_shop_seller
# apk更新日志
update_msg = "修复已知问题\n提升用户体验"
...
def upload(_app_type, market_channel, _apk_dir_path):
    marketUploadObj = None
    if market_channel == MarketChannel.xiaomi:
        marketUploadObj = XiaomiMarketUpload(market_channel, _app_type, apk_dir_path)
    elif market_channel == MarketChannel.oppo:
        marketUploadObj = OppoMarketUpload(market_channel, _app_type, apk_dir_path)
    ...
    if len(apk_file) == 0:
        raise Exception("_app_type = %s market_channel = %s, cannot find the apk file in %s" %
                        (_app_type, market_channel, _apk_dir_path))
    marketUploadObj.upload(apk_file, update_msg, isAutoCommit)
    print("upload end,_app_type = %s market_channel = %s, apk_file = %s" %
          (_app_type.value, market_channel.value, apk_file))
...
for market in market_list:
    try:
        thread = threading.Thread(target=upload, args=(app_type, market, apk_dir_path))
        thread.start()
        thread_list.append(thread)
        # upload(app_type, market, apk_dir_path)
    except Exception as e:
        print("upload %s %s occur exception:" % (app_type.value,  market.value))
        traceback.print_exc()
